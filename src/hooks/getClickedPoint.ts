export default function getClickedPoint(x: number, y: number, dataOnPlot: any) {
    const allPoints = Array.from(document.querySelectorAll(".custom-dot"));

    for (let i = 0; i < allPoints.length; i++) {
        const { chartX, chartY, xValue, yValue, radius } = (allPoints[i] as any).dataset;
        ...
        if (distance <= Number(radius)) {
            const dataPoint = dataOnPlot.find(
                (d: any) => d.x === Number(xValue) && d.y === Number(yValue)
            );
            if (dataPoint) {
                return dataPoint;
            }
